...
class GenderInferenceAgent:
    def __init__(self, low_conf_threshold=0.3, high_conf_threshold=0.7):
        """
    ...
    def infer_gender(self, 
        ...
        Strict opt-out: if explicit gender exists, skip inference entirely
        """
        # STRICT OPT-OUT: Skip inference if explicit gender provided
        if explicit_gender:
            print(f"Explicit gender provided: {explicit_gender}. Skipping inference.")
            return None
...
if __name__ == "__main__":
    ...
    print("- Strict opt-out when explicit gender provided")
    print("-" * 60)
    
    # Initialize agent
    agent = GenderInferenceAgent()
    
    # Test cases
    test_cases = [
        {'name': 'Michael Johnson', 'sport_team': 'UCLA Football', 'profile_image': './data/image1.jpg', 'explicit_gender': None},
        {'name': 'Alex Morgan', 'sport_team': 'US Women\'s Soccer', 'profile_image': './data/image2.jpg', 'explicit_gender': None},
        {'name': 'Alex Wilson', 'sport_team': 'Track and Field', 'profile_image': './data/image3.jpg', 'explicit_gender': None},  # Ambiguous name + mixed sport
        {'name': 'Jordan Lee', 'sport_team': 'Swimming', 'profile_image': './data/image4.jpg', 'explicit_gender': None},
        {'name': 'Sarah Johnson', 'sport_team': 'Men\'s Basketball Team', 'profile_image': './data/image5.jpg', 'explicit_gender': None},  # Mis-assigned team
        {'name': 'Chris Taylor', 'sport_team': 'Tennis Club', 'profile_image': None, 'explicit_gender': None},
        {'name': 'Test User', 'sport_team': 'Soccer', 'profile_image': None, 'explicit_gender': 'female'},  # Explicit gender - should skip
    ]
    ...
    for test in test_cases:
        print(f"\nAthlete: {test['name']}")
        print(f"Team: {test['sport_team']}")
        print(f"Image: {test.get('profile_image', 'None')}")
        print(f"Explicit Gender: {test.get('explicit_gender', 'None')}")
        
        result = agent.infer_gender(
            athlete_name=test['name'],
            sport_team=test['sport_team'],
            profile_pic_path=test.get('profile_image'),
            explicit_gender=test.get('explicit_gender')
        )
        ...
        if result:
            ...
            print("Inference skipped (explicit gender provided)")
